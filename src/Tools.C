...
{
  //TODO copyBits no check that length > 0
  if(srclow >= 0 && dstlow >= 0 && ((length + srclow) - 1) < 64 && 
    ((length + dstlow) - 1) < 64 && (length > 0)){
      uint64_t temp = getBits(source, srclow, ((length + srclow) - 1));
      dest = clearBits(dest, dstlow, ((length + dstlow) - 1));
      temp = temp << dstlow;
      return dest | temp; 
    }
   return dest;
...
{
  //set the bits of the byte given
...
{
  //should only have to get the highest order bit to determine whether pos or neg
  //if highest order bit is a 1 it is neg if 0 pos
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
  ...
  //      the result should be positive, otherwise an overflow occurred
    ...
    if((sign(op1) && sign(op2) && (!sign(result))) || ((!sign(op1)) && (!sign(op2)) && sign(result))){
      return true;
    }
  return false;
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
  ...
  if(((!sign(op1)) && sign(op2) && (!sign(result))) 
    || (sign(op1) && (!sign(op2)) && sign(result))){
      return true;
  }
  return false;
}
